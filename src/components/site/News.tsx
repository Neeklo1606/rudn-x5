...
const news: [string, string, string, string][] = [
  ["События", "15 июня 2026", "День открытых дверей программы ИИ", "Более 200 абитуриентов посетили кампус РУДН и лаборатории X5 Tech, чтобы узнать о программе..."],
  ["Партнёрство", "10 июня 2026", "X5 Tech открывает лабораторию в РУДН", "Новая лаборатория оснащена GPU-кластером и интерактивными рабочими станциями для студентов..."],
  ["Образование", "5 июня 2026", "Первый выпуск программы — результаты", "85% выпускников получили офферы в X5 Tech и другие технологические компании ещё до защиты диплома..."],
];
    ...
    <section id="news" style={{ background: "#fff" }} className="section">
      <div className="container">
        ...
        <div className="news-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 28 }}>
          {news.map(([cat, date, title, desc], i) => (
            <motion.article key={title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5 }}
              ...
              <div style={{ height: 200, background: patterns[i] }} />
