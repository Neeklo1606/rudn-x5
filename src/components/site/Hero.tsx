...
export default function Hero() {
  ...
  return (
    <section ref={ref} id="hero" style={{ background: "#fff", minHeight: "calc(100vh - 72px)", padding: "120px 0", marginTop: 72, clipPath: "polygon(0 0, 100% 0, 100% 95%, 0 100%)" }}>
      <div className="container">
        <div className="hero-grid" style={{ display: "flex", gap: 64, alignItems: "stretch" }}>
          <div style={{ flex: "0 0 55%", display: "flex", flexDirection: "column", gap: 40 }}>
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.24 }}
              style={{ fontWeight: 800, fontSize: 64, lineHeight: 1.05, letterSpacing: -1, color: "#272727", maxWidth: 620 }} className="hero-h1">
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.36 }}
              style={{ fontSize: 18, lineHeight: 1.7, color: "#6B6B6B", maxWidth: 520 }}>
              Математика, программирование и реальные ИИ-задачи в партнёрстве с X5 Tech — работай с первого курса над проектами, которые приносят бизнесу измеримый результат, а тебе — опыт и кейсы в портфолио.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.48 }}
              ...
              <a href="#apply" className="btn-primary" style={{ background: "#B6E835", color: "#272727", fontWeight: 600, fontSize: 16, padding: "18px 36px", borderRadius: 999, border: "none", cursor: "pointer", transition: "all 250ms ease", display: "inline-block" }}>
                Подать заявку
              </a>
              <a href="#apply" className="btn-outline" style={{ background: "transparent", border: "2px solid #272727", color: "#272727", fontWeight: 600, fontSize: 16, padding: "16px 34px", borderRadius: 999, cursor: "pointer", transition: "all 250ms ease", display: "inline-block" }}>
                Проконсультироваться
              </a>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.6 }}
              className="stats-row" style={{ display: "flex", gap: 48, flexWrap: "wrap", marginTop: 8 }}>
              {[
                ...
                <div key={st.l}>
                  <div style={{ fontWeight: 700, fontSize: 36, color: "#B6E835", lineHeight: 1 }}>
                    {inView ? <Counter to={st.n} suffix={st.s} /> : 0}
                  </div>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "#A3A3A3", textTransform: "lowercase", letterSpacing: 0.5, marginTop: 8 }}>{st.l}</div>
                </div>
              ))}
            </motion.div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, x: 60 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ type: "spring", stiffness: 120, damping: 20, delay: 0.3 }}
            style={{ flex: "0 0 calc(45% - 64px)", background: "#272727", borderRadius: 32, padding: "48px 40px", position: "relative", boxShadow: "var(--shadow-dark-card)", overflow: "hidden" }} className="hero-dark-card">
            <div style={{ position: "absolute", top: 40, left: 40, width: 60, height: 3, background: "#B6E835" }} />
            <h3 style={{ fontWeight: 700, fontSize: 26, lineHeight: 1.3, color: "#fff", marginTop: 48 }}>
              AI-практика начинается на 1 курсе, а не после диплома
            </h3>
            <ul style={{ listStyle: "none", marginTop: 32, display: "flex", flexDirection: "column", gap: 20 }}>
              {["Реальные проекты X5 Tech с первого семестра", "Оплачиваемая стажировка в X5 Tech со 2 курса", "Кейсы в портфолио вместо абстрактных задач"].map((t) => (
                <li key={t} style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                  <span style={{ width: 24, height: 24, borderRadius: "50%", border: "2px solid #B6E835", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Check size={14} color="#B6E835" strokeWidth={3} />
                  </span>
                  <span style={{ fontSize: 16, lineHeight: 1.5, color: "rgba(255,255,255,0.85)" }}>{t}</span>
                </li>
              ))}
            </ul>
            <div style={{ position: "absolute", bottom: 0, left: 0, width: "100%", height: 4, background: "linear-gradient(90deg, #B6E835, #A79FFF)", borderRadius: "0 0 32px 32px" }} />
          </motion.div>
        </div>
      </div>
      <style>{`
